...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], args.hidden_size).to(device=self.device)#Critic Network，Q网络
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)

        self.critic_target = QNetwork(num_inputs, action_space.shape[0], args.hidden_size).to(self.device)#Target Q Network
    ...
    def select_action(self, state, eval=False):
        ...
        if eval == False:
            ...
            _, _, action = self.policy.sample(state)#action, log_prob, torch.tanh(mean)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        with torch.no_grad():
            ...
            next_q_value = reward_batch + mask_batch * self.gamma * (min_qf_next_target)#r(st,at) + γ(𝔼st+1~p[V(st+1)]))
